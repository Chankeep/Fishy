...
private:
	...
	static entt::resource<Texture> loadGltfTexture(const LoadContext& ctx, size_t textureIndex, vk::Format format,
												   const std::string& texName);
	static entt::resource<Material> loadGltfMaterial(const LoadContext& ctx, size_t materialIndex);
	static void processGltfNode(LoadContext& ctx, size_t nodeIndex, Entity parentEntity = Entity{});

	static void applyNodeTransform(const fastgltf::Node& node, TransformComponent& transform);
};
