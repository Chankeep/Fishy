...
void Window::init(const Properties& properties) {
	...
	if (!_window) {
		FISHY_LOG_ERROR("Failed to create GLFW window: {} ({}x{})", properties.title, properties.width,
							   properties.height);
		throw std::runtime_error("Failed to create GLFW window");
	}

	FISHY_LOG_TRACE("Created GLFW window: {} ({}x{})", properties.title, properties.width, properties.height);
...
void Window::KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (key == GLFW_KEY_F11 && action == GLFW_PRESS) {
		...
		if (win->_isFullscreen) {
			...
			FISHY_LOG_INFO("Exited fullscreen mode");
		} else {
			...
			FISHY_LOG_INFO("Entered fullscreen mode: {}x{} @ {}Hz", mode->width, mode->height,
								  mode->refreshRate);
		}
	}
}
